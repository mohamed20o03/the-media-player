document.addEventListener('DOMContentLoaded', function () {
    ...
    var audioSource = audioContext.createMediaElementSource(media);

    audioSource.connect(audioContext.destination);

    // Create a DelayNode with feedback
    var delayNode = audioContext.createDelay();
    var feedbackGain = audioContext.createGain();
    delayNode.delayTime.value = 0.1; // Set the delay time (in seconds)
    feedbackGain.gain.value = 0; // Set the feedback gain (adjust as needed)

    // Connect the delay node's output to the feedback gain
    delayNode.connect(feedbackGain);
    // Connect the feedback gain back to the delay node's input
    feedbackGain.connect(delayNode);

    // JavaScript logic to handle echo effect buttons
    var addEchoButton = document.getElementById('addEchoButton');
    var removeEchoButton = document.getElementById('removeEchoButton');

    // Event listener for "Add Echo" button
    addEchoButton.addEventListener('click', function () {
        // Connect the original audio source to the delay node
        audioSource.connect(delayNode);
        // Connect the delay node to the audio context destination (speakers)
        delayNode.connect(audioContext.destination);
    });

    // Event listener for "Remove Echo" button
    removeEchoButton.addEventListener('click', function () {
        audioSource.disconnect(delayNode);
        // Connect the original audio source directly to the audio context destination
        audioSource.connect(audioContext.destination);
        // Disconnect the delay node from the audio context destination
